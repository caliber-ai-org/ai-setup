...
export async function learnObserveCommand(options: { failure?: boolean; prompt?: boolean }) {
  try {
    ...
    if (options.prompt) {
      const event: PromptEvent = {
        ...
        prompt_content: sanitizeSecrets(
          String(hookData.prompt_content || hookData.content || hookData.prompt || ''),
        ),
    ...
    if (eventsSinceLastAnalysis >= INCREMENTAL_INTERVAL) {
      try {
        const { resolveCaliber, resolveCaliberForSpawn } =
          await import('../lib/resolve-caliber.js');
        const { spawn } = await import('child_process');
        const logPath = path.join(getLearningDir(), LEARNING_FINALIZE_LOG);
        if (!fs.existsSync(getLearningDir())) fs.mkdirSync(getLearningDir(), { recursive: true });
        const logFd = fs.openSync(logPath, 'a');
        const finalizeArgs = ['learn', 'finalize', '--auto', '--incremental'];

        // Prefer spawning node + script directly to avoid Windows .cmd shim
        // issues (ENOENT without shell, visible cmd window with shell).
        const resolved = resolveCaliberForSpawn();
        if (resolved) {
          const [nodeBin, scriptPath] = resolved;
          spawn(nodeBin, [scriptPath, ...finalizeArgs], {
            detached: true,
            stdio: ['ignore', logFd, logFd],
            windowsHide: true,
          }).unref();
        } else {
          // Fallback: use shell resolution for .cmd shims / npx
          const bin = resolveCaliber();
          spawn(bin, finalizeArgs, {
            detached: true,
            stdio: ['ignore', logFd, logFd],
            shell: true,
            windowsHide: true,
